...
class SettlementTest {
    ...
    void createNew_shouldCalculateFeesAndNetAmountCorrectly() {
        Settlement settlement = Settlement.createNew(tradeId, valueDate, grossAmount,
                Settlement.Status.PENDING, messageId);
    ...
    void rehydrate_shouldSucceedWhenNetAmountMatchesGrossMinusFees() {
        ...
        Settlement settlement = Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                fees, net, Settlement.Status.SETTLED, messageId);
    ...
    void rehydrate_shouldThrowWhenNetAmountDoesNotMatch() {
        ...
        assertThatThrownBy(() ->
                Settlement.rehydrate(UUID.randomUUID(), tradeId, valueDate, grossAmount,
                        fees, wrongNet, Settlement.Status.FAILED, messageId))
    ...
    void statusChecks_shouldReturnCorrectBooleans() {
        Settlement pending = Settlement.createNew(tradeId, valueDate, grossAmount,
                Settlement.Status.PENDING, messageId);
        Settlement settled = Settlement.createNew(tradeId, valueDate, grossAmount,
                Settlement.Status.SETTLED, messageId);
        Settlement failed = Settlement.createNew(tradeId, valueDate, grossAmount,
                Settlement.Status.FAILED, messageId);
    ...
    void equalsAndHashCode_shouldBeBasedOnId() {
        ...
        Settlement s1 = Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                fees, net, Settlement.Status.PENDING, messageId);
        Settlement s2 = Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                fees, net, Settlement.Status.PENDING, messageId);
    ...
    void validate_shouldThrowWhenAnyArgumentIsNull() {
        ...
        assertThatThrownBy(() ->
                Settlement.rehydrate(null, tradeId, valueDate, grossAmount,
                        fees, net, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("ID cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, null, valueDate, grossAmount,
                        fees, net, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Trade ID cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, null, grossAmount,
                        fees, net, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Value Date cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, valueDate, null,
                        fees, net, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Gross Amount cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                        null, net, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Fees cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                        fees, null, Settlement.Status.PENDING, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Net Amount cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                        fees, net, null, messageId))
                .isInstanceOf(NullPointerException.class)
                .hasMessageContaining("Status cannot be null");

        assertThatThrownBy(() ->
                Settlement.rehydrate(id, tradeId, valueDate, grossAmount,
                        fees, net, Settlement.Status.PENDING, null))
                .isInstanceOf(NullPointerException.class)
