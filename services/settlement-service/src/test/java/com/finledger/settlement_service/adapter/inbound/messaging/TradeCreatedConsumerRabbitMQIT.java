...
class TradeCreatedConsumerRabbitMQIT {
  ...
  static RabbitMQContainer rabbit =
      new RabbitMQContainer("rabbitmq:3.12-management").withExposedPorts(5672, 15672);
  ...
  void shouldConsumeMessageFromRabbitMQ() throws Exception {
    ...
    rabbitTemplate.convertAndSend(
        ...
        json,
        m -> {
          m.getMessageProperties().setHeader("x-correlation-id", "corr-123");
          return m;
        });
    // Assert: wait until the listener processes it
    await()
        .atMost(5, SECONDS)
        .untilAsserted(() -> verify(useCase).execute(any(TradeCreatedEventDto.class)));
  }
}
