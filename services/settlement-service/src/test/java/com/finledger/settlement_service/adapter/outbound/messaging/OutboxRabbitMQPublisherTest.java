...
class OutboxRabbitMQPublisherTest {
  ...
  void setUp() {
    ...
    RabbitMQBindingsProperties.BindingProperties tradeBinding =
        new RabbitMQBindingsProperties.BindingProperties();
    ...
    RabbitMQBindingsProperties.BindingProperties settlementBinding =
        new RabbitMQBindingsProperties.BindingProperties();
  ...
  void processBatch_shouldSendAndMarkSent() {
    ...
    verify(outboxRepo)
        .updateDeliveryState(
            eq(event.getId()), eq(OutboxStatus.SENT), eq(0), isNull(), isNull(), any());
