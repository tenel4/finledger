...
class TradeTest {
  ...
  void rehydrate_shouldUseProvidedIdAndTradeTime() {
    ...
    Trade trade =
        Trade.rehydrate(id, "AAPL", Trade.Side.SELL, quantity, price, buyerId, sellerId, tradeTime);
  ...
  void shouldThrow_whenBuyerAndSellerAreSame() {
    ...
    assertThatThrownBy(
            () -> Trade.createNew("AAPL", Trade.Side.BUY, quantity, price, sameId, sameId))
  ...
  void equalsAndHashCode_shouldBeBasedOnId() {
    UUID id = UUID.randomUUID();
    Trade t1 =
        Trade.rehydrate(id, "AAPL", Trade.Side.BUY, quantity, price, buyerId, sellerId, tradeTime);
    Trade t2 =
        Trade.rehydrate(id, "AAPL", Trade.Side.SELL, quantity, price, buyerId, sellerId, tradeTime);
