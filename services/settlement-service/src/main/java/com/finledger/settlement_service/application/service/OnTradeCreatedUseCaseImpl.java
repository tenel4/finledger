...
public class OnTradeCreatedUseCaseImpl implements OnTradeCreatedUseCase {
  ...
  public OnTradeCreatedUseCaseImpl(
  ...
  public void execute(TradeCreatedEventDto event) {
    ...
    Settlement settlement =
        Settlement.createNew(
            event.tradeId(), valueDate, grossAmount, Settlement.Status.PENDING, event.eventId());
    settlementRepositoryPort.save(settlement);

    SettlementCreatedEvent domainEvent =
    ...
    try {
      ...
      throw new SettlementCreationException(
          "Failed to create settlement due to event persistence error", e);
    }
  }
}
