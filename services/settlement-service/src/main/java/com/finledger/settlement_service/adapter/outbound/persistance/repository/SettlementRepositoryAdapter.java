...
public class SettlementRepositoryAdapter implements SettlementRepositoryPort {
  ...
  public List<Settlement> find(Settlement.Status status, LocalDate date) {
    if (status != null && date != null) {
      return repo.findByStatusAndValueDate(status, date).stream().map(this::toDomain).toList();
    } else if (status != null) {
      return repo.findByStatus(status.name()).stream().map(this::toDomain).toList();
    } else if (date != null) {
      return repo.findByValueDate(date).stream().map(this::toDomain).toList();
    } else {
