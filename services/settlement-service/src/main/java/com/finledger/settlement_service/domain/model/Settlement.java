...
public final class Settlement {
  ...
  public static Settlement createNew(
      UUID tradeId, LocalDate valueDate, Money grossAmount, Status status, UUID messageId) {
    Money fees = grossAmount.times(DEFAULT_FEE_RATE); // TODO hard coded
    Money netAmount = grossAmount.subtract(fees);
    return new Settlement(
        UUID.randomUUID(), tradeId, valueDate, grossAmount, fees, netAmount, status, messageId);
  }
