...
public class OutboxRabbitMQPublisher {
  ...
  public OutboxRabbitMQPublisher(
    ...
    metrics.gauge(
        "outbox.backlog.total",
        this,
        p -> p.outboxRepo.countByStatuses(List.of(OutboxStatus.PENDING, OutboxStatus.RETRY)));
    metrics.gauge(
        "outbox.backlog.pending", this, p -> p.outboxRepo.countByStatus(OutboxStatus.PENDING));
    metrics.gauge(
        "outbox.backlog.retry", this, p -> p.outboxRepo.countByStatus(OutboxStatus.RETRY));
    metrics.gauge("outbox.backlog.dead", this, p -> p.outboxRepo.countByStatus(OutboxStatus.DEAD));
  }
  ...
  public void scheduledFlush() {
    ...
    log.info(
        "Starting outbox flush: maxBatches={} batchSize={} correlationId={} traceId={}",
        props.getMaxBatchesPerRun(),
        props.getBatchSize(),
        correlationId,
        traceId);
    ...
    metrics
        .timer("outbox.flush.duration")
        .record(System.currentTimeMillis() - start, TimeUnit.MILLISECONDS);

    log.info(
        "Completed outbox flush: batches={} sent={} retries={} dead={} durationMs={} correlationId={} traceId={}",
        batches,
        totalProcessed,
        totalRetries,
        totalDead,
  ...
  protected BatchResult processBatch(List<OutboxEventEntity> batch) {
    ...
    for (OutboxEventEntity e : batch) {
      ...
      if (binding == null) {
        log.error(
            "No binding found for event type={} correlationId={} traceId={}",
            e.getType(),
            correlationId,
            traceId);
        continue;
      ...
      try {
        ...
        outboxRepo.updateDeliveryState(
            e.getId(), OutboxStatus.SENT, e.getRetryCount(), null, null, Instant.now());
        metrics
            .counter(
                "outbox.sent",
                METRICS_TAG_EXCHANGE,
                binding.getExchange(),
                "rk",
                binding.getRoutingKey())
            .increment();
        ...
        log.debug(
            "Outbox event sent: id={} type={} exchange={} rk={} correlationId={} traceId={}",
            e.getId(),
            e.getType(),
            binding.getExchange(),
            binding.getRoutingKey(),
        ...
        if (nextRetry > props.getMaxRetries()) {
          ...
          outboxRepo.updateDeliveryState(
              e.getId(), OutboxStatus.DEAD, nextRetry - 1, null, err, Instant.now());
          metrics
              .counter(
                  "outbox.dead",
                  METRICS_TAG_EXCHANGE,
              ...
              .increment();
          log.error(
              "Poison message moved to dead_outbox_event id={} type={} exchange={} rk={} error={} correlationId={} traceId={}",
              e.getId(),
              e.getType(),
              binding.getExchange(),
              binding.getRoutingKey(),
          ...
          outboxRepo.updateDeliveryState(
              e.getId(), OutboxStatus.RETRY, nextRetry, nextAttempt, err, Instant.now());
          metrics
              .counter(
                  "outbox.retry",
                  METRICS_TAG_EXCHANGE,
              ...
              .increment();
          log.warn(
              "Outbox publish failed; scheduling retry={} in {} ms id={} type={} exchange={} rk={} error={} correlationId={} traceId={}",
              nextRetry,
              backoff,
              e.getId(),
              e.getType(),
  ...
  private long computeBackoff(int attempt) {
    double delay =
        props.getBackoffInitialMs()
            * Math.pow(props.getBackoffMultiplier(), Math.max(0, attempt - 1));
