...
public class TradeController {
  ...
  public TradeController(
      CreateTradeUseCase createTrade,
  ...
  public ResponseEntity<CreateTradeResponse> createTrade(
      @RequestBody @Valid CreateTradeRequest req) {
    var command =
        new CreateTradeUseCase.Command(
            req.symbol(),
            req.side(),
            req.quantity(),
            req.price(),
  ...
  public ResponseEntity<List<GetTradesResponse>> getTrades(
    ...
    List<Trade> trades =
        getTrades.execute(symbol, fromI, toI, side); // TODO decouple domain model and rest
    var result = trades.stream().map(TradeMapper::toResponse).toList();
  ...
  public ResponseEntity<List<GetSettlementsResponse>> listSettlements(
    ...
    List<Settlement> settlements =
        getSettlements.execute(status, date); // TODO decouple domain model and rest
    var result = settlements.stream().map(SettlementMapper::toResponse).toList();
    return ResponseEntity.ok(result);
  }
