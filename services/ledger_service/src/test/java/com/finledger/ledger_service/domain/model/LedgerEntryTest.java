...
class LedgerEntryTest {
    ...
    void isDebit_shouldDelegateToAmount() {
        LedgerEntry debitEntry = LedgerEntry.createNew(
                UUID.randomUUID(),
                DEBIT_5_USD,
                LedgerEntryReferenceType.TRADE,
                UUID.randomUUID(),
    ...
    void isCredit_shouldDelegateToAmount() {
        LedgerEntry creditEntry = LedgerEntry.createNew(
                UUID.randomUUID(),
                CREDIT_10_USD,
                LedgerEntryReferenceType.TRADE,
                UUID.randomUUID(),
    ...
    void equalsAndHashCode_shouldDependOnIdOnly() {
        UUID id = UUID.randomUUID();
        LedgerEntry e1 = LedgerEntry.rehydrate(
                id, Instant.now(), UUID.randomUUID(), CREDIT_10_USD,
                LedgerEntryReferenceType.TRADE, UUID.randomUUID(), UUID.randomUUID()
        );
        LedgerEntry e2 = LedgerEntry.rehydrate(
                id, Instant.now(), UUID.randomUUID(), DEBIT_5_USD,
                LedgerEntryReferenceType.SETTLEMENT, UUID.randomUUID(), UUID.randomUUID()
        );
        LedgerEntry e3 = LedgerEntry.rehydrate(
                UUID.randomUUID(), Instant.now(), UUID.randomUUID(), CREDIT_10_USD,
                LedgerEntryReferenceType.TRADE, UUID.randomUUID(), UUID.randomUUID()
        );
    ...
    void rehydrate_shouldThrowWhenAccountIdIsNull() {
        UUID id = UUID.randomUUID();
        assertThatThrownBy(() ->
                LedgerEntry.rehydrate(id, NOW, null, AMOUNT, REF_TYPE, REF_ID, MSG_ID)
        )
                .isInstanceOf(NullPointerException.class)
