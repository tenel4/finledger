...
class OnSettlementCreatedUseCaseImplTest {
    ...
    void execute_whenMessageIsNew_shouldCreateAndSaveTwoEntries() {
        ...
        verify(ledgerRepo, times(1)).saveAll(argThat(entries -> {
            ...
            assertThat(buyerEntry.amount())
                    .isEqualTo(SignedMoney.of(netAmount.negate(), Currency.getInstance(currency)));
            assertThat(buyerEntry.referenceType()).isEqualTo(LedgerEntryReferenceType.SETTLEMENT);
            ...
            assertThat(sellerEntry.referenceType()).isEqualTo(LedgerEntryReferenceType.SETTLEMENT);
    ...
    void execute_whenMessageAlreadyProcessed_shouldDoNothing() {
        ...
        useCase.execute(messageKey, UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                BigDecimal.TEN, "USD");
