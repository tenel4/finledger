...
public class SettlementConsumerRabbitMQ {
    ...
    public record SettlementCreatedEvent(UUID eventId, Instant occurredAt, UUID settlementId, UUID buyerAccountId,
                                         UUID sellerAccountId, BigDecimal netAmount, String currency, String eventType,
    ...
    public void onMessage(String json) {
        try {
            ...
            useCase.execute(event.eventId(), event.settlementId(), event.buyerAccountId(),
                    event.sellerAccountId(), event.netAmount(), event.currency());
